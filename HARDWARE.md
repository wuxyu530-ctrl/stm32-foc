...
6. ~~M2 开环 SVPWM 上板~~ ✅ 2026-09-18，见 §8.7
...
### 8.7 M2 开环 SVPWM（2026-09-18，首次接电机）

固件：主循环 1 kHz 推进电角度，`U_dq = (0, U_AMP)` → `foc_inv_park` → `foc_svpwm(d_max = 0.956)` → CCR。
Vdc 暂用固定 12 V。上电先输出零矢量再开 PWM。限流 0.5 A，全程未触发。

| `F_ELEC` | `U_AMP` | 机械转速 | 现象 |
|---|---|---|---|
| 2 Hz | 1.0 V | 0.29 r/s | **平稳**，手拧转子有明显弹性回位 |
| 10 Hz | 1.0 V | 1.4 r/s | 略抖（开环同步机摆振，电压裕量缩小） |
| 30 Hz | 1.0 V | 4.3 r/s | 抖动明显但未失步 |
| 30 Hz | 2.5 V | 4.3 r/s | **平稳** |

结论：`core/` 的逆 Park + SVPWM 在真实功率级上正确；现象符合 V/f 规律（反电动势随转速吃掉电压裕量，
需要按转速抬高 U）。1 kHz 更新在 30 Hz 时每步 10.8° 电角度也贡献部分抖动，M3 后控制移入 20 kHz ADC 中断。

**M2 的意义**：从 `foc_clarke` 的第一行到转子转起来，`core/` 全部由自己实现并验证。
